        ...
        <ol className="grid grid-cols-4 gap-1 2xl:flex 2xl:flex-col 2xl:gap-0">
          {homeSections.map((section, index) => {
            ...
            return (
              <li
                key={section.id}
                className="flex flex-col items-center 2xl:items-start"
              >
                <a
                  aria-current={isActive ? "location" : undefined}
                  className={cn(
                    "group flex w-full min-w-0 flex-col items-center gap-1.5 rounded-[1.35rem] px-2 py-2 text-center transition-colors duration-200 2xl:flex-row 2xl:justify-start 2xl:gap-3 2xl:px-3 2xl:py-2.5",
                    isActive
                ...
                >
                  <span
                    className={cn(
                      "relative flex shrink-0 items-center justify-center rounded-full border transition-all duration-300 2xl:size-8",
                      isActive
                        ? "size-7 scale-[1.18] border-black/15 bg-white shadow-[0_10px_24px_-16px_rgba(15,23,42,0.9)] 2xl:size-8 2xl:scale-[1.22] dark:border-white/15 dark:bg-white/12 dark:shadow-[0_10px_24px_-16px_rgba(2,6,23,0.9)]"
                        : isComplete
                          ? "size-6 border-emerald-300/70 bg-emerald-50 2xl:size-7 dark:border-cyan-400/40 dark:bg-cyan-400/10"
                          : "size-6 border-black/10 bg-white/45 2xl:size-7 dark:border-white/12 dark:bg-white/8"
                    )}
                  >
                    {isActive && (
                      <span
                    ...
                    <span
                      className={cn(
                        ...
                        isActive
                          ? "size-3.5 bg-[linear-gradient(135deg,#eab308,#27ffc3)] 2xl:size-4 dark:bg-[linear-gradient(135deg,#22d3ee,#818cf8)]"
                          : isComplete
                  ...
                  <span className="text-[0.7rem] leading-tight font-medium text-current sm:text-xs 2xl:text-sm">
                    {t(section.labelKey)}
                  </span>
                </a>

                {index < homeSections.length - 1 ? (
                  <div className="hidden w-full py-0.5 2xl:flex 2xl:justify-start">
                    <span
                      aria-hidden="true"
                    ...
                    >
                      <span
                        className={cn(
                          "block h-full w-0.5 origin-top rounded-full bg-[linear-gradient(180deg,#eab308,#27ffc3)] transition-[transform] duration-500 ease-out dark:bg-[linear-gradient(180deg,#22d3ee,#818cf8)]",
